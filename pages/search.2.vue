  ...
  >
    <template slot="items" slot-scope="props">
     <td>{{ props.item.b_id }}</td>
      <td class="text-xs-center">{{ props.item.b_title }}</td>
      <td class="text-xs-center">{{ props.item.b_author }}</td>
...
export default {
      ...
      headers: [
        {
          ...
          { text: 'b_title', value: 'b_title', align: 'center' },
          { text: 'b_author', value: 'b_author', align: 'center' },
          { text: 'b_password', value: 'b_password', align: 'center' },
      ],
      desserts: [],
      member :'',
    };
  ...
  methods: {
    ...
    async getstudent() {
      let res = await this.$http.get('http://127.0.0.1/php-test2/list-books.php')
